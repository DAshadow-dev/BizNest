...
const CreateTransactionScreen = () => {
  ...
  const updateProductQuantity = (productId: string, quantity: number) => {
    if (quantity <= 0) {
      ...
      // Update quantity
      setSelectedProducts(selectedProducts.map((p) => (p._id === productId ? { ...p, quantity } : p)))
    }
  }
  ...
  const handleSubmitTransaction = () => {
    ...
    // Create transaction data - ensure all values are defined
    const transactionData = {
      customerId: selectedCustomer._id || "", // Add fallback for undefined
      products: selectedProducts.map((p) => ({
        productId: p._id || "", // Add fallback for undefined
        quantity: p.quantity || 1, // Add fallback for undefined
      })),
      totalAmount: totalAmount || 0, // Add fallback for undefined
    }
    ...
    const hasInvalidProducts = transactionData.products.some((p) => !p.productId)
      ...
      payload: {
        ...
        onFailed: () => {
          Toast.show({
            type: "error",
            text1: "Fail!",
            text2: "Create transaction unsuccessfully",
            position: "top",
            visibilityTime: 2000,
          })
        },
      ...
      <FormProvider {...methods}>
        ...
        >
          ...
          <View style={styles.formInput}>
            ...
            {showCustomerResults && (
              <View style={styles.searchResults}>
                <ScrollView style={{ maxHeight: verticalScale(150) }}>
                  {customers.length > 0 ? (
                    customers.map((item) => (
                      <TouchableOpacity
          ...
          <View style={styles.formInput}>
            ...
            <TouchableOpacity style={styles.addProductButton} onPress={() => setShowProductList(!showProductList)}>
              <Text style={styles.addProductButtonText}>{showProductList ? "Hide Products" : "Add Product"}</Text>
            </TouchableOpacity>
              ...
              <View style={styles.selectedProductsContainer}>
                ...
                <ScrollView style={{ maxHeight: verticalScale(200) }}>
                  {selectedProducts.map((item) => (
                    <View key={item._id?.toString() || Math.random().toString()} style={styles.selectedProductItem}>
                      <View style={{ flex: 1 }}>
          ...
          <View style={styles.buttonContainer}>
            ...
            <TouchableOpacity style={styles.confirmButton} onPress={handleSubmitTransaction}>
